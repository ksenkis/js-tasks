...
function analyzeBookSalesCategory(booksArray) {
  ...
  booksArray.forEach((book) => {
    const record = categoryAmount.find(
...
function analyzeStudents(students) {
  const filteredStudents = students.filter(
    ({ grades }) =>
      grades.reduce((sum, grade) => sum + grade) / grades.length >= 4,
  );
...
function analyzeReviews(texts, n) {
  ...
  return (
    longReviews.reduce((acc, item) => acc + item.rating, 0) /
    longReviews.length.toFixed(2)
  );
}

function routeTotals(routes, minSegments) {
  const moreSegments = routes.filter(
    ({ segments }) => segments.length >= minSegments,
  );

  return moreSegments.map(({ id, segments }) => {
    return {
      id,
      total: segments.reduce((acc, item) => acc + item.distance, 0),
    };
  });
}
...
function balanceByType(tx, typeKey, amountKey) {
  ...
  const total =
    "income" in result && "expense" in result
